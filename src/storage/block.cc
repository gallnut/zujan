...
Block::Block(std::string_view contents) : data_(contents.data()), size_(contents.size())
{
...
Block::Iterator::Iterator(const char *data, uint32_t restarts, uint32_t num_restarts)
    : data_(data), restarts_(restarts), num_restarts_(num_restarts), current_(restarts_), restart_index_(num_restarts_)
{
}
...
void Block::Iterator::ParseNextKey()
{
    ...
    while (restart_index_ + 1 < num_restarts_ &&
           DecodeFixed32(data_ + restarts_ + (restart_index_ + 1) * sizeof(uint32_t)) <= current_)
    {
        ++restart_index_;
    }
}
...
void Block::Iterator::Seek(std::string_view target)
{
    ...
    while (left < right)
    {
        ...
        uint32_t    region_offset = DecodeFixed32(data_ + restarts_ + mid * sizeof(uint32_t));
        uint32_t    shared, non_shared, value_length;
        const char *key_ptr = GetVarint32PtrFallback(data_ + region_offset, data_ + restarts_, &shared);
