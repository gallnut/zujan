...
// itself:
std::expected<std::unique_ptr<LSMStore>, Error> LSMStore::Open() noexcept {
  auto store = std::unique_ptr<LSMStore>(new LSMStore());
  auto res = store->Init();
  if (!res)
    return std::unexpected(Error{ErrorCode::IOError, "LSMStore Init failed"});
...
std::expected<void, Error> LSMStore::Init() noexcept {
  ...
  wal_ = std::make_unique<WAL>(*io_ctx_, "wal.log");
...
std::expected<void, Error> LSMStore::Put(const std::string &key,
...
LSMStore::Get(const std::string &key) noexcept {
  // 1. MemTable GET
