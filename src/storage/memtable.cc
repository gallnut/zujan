...
MemTable::MemTable() : comparator_(), arena_(), table_(comparator_, &arena_), sequence_(1) {}
...
void MemTable::Put(std::string_view key, std::string_view value)
{
    ...
    const size_t encoded_len = 5 + internal_key_size + 5 + val_size;  // Max 5 bytes for varints
...
std::optional<std::string> MemTable::Get(std::string_view key) const
{
    ...
    if (iter.Valid())
    {
        ...
        const char *entry_key_ptr = GetVarint32Ptr(entry, entry + 5, &entry_internal_key_size);
        uint32_t    entry_user_key_size = entry_internal_key_size - 8;

        // Check if user key matches
        if (entry_user_key_size == key_size && std::memcmp(entry_key_ptr, key.data(), key_size) == 0)
        {
