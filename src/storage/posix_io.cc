...
std::expected<int, Error> PosixIOContext::ReadAligned(int fd,
                                                      std::span<char> buf,
...
PosixIOContext::WriteAligned(int fd, std::span<const char> buf,
                             off_t offset) noexcept {
...
PosixIOContext::ReadAsync(int fd, std::span<char> buf, off_t offset) noexcept {
  return std::async(std::launch::async, [this, fd, buf, offset]() {
    return ReadAligned(fd, buf, offset);
  });
}

std::future<std::expected<int, Error>>
PosixIOContext::WriteAsync(int fd, std::span<const char> buf,
                           off_t offset) noexcept {
  return std::async(std::launch::async, [this, fd, buf, offset]() {
    return WriteAligned(fd, buf, offset);
  });
}
