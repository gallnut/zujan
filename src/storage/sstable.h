...
public:
  ...
  static std::expected<std::unique_ptr<SSTable>, Error>
  Open(const TableBuilderOptions &options, IOContext &io_ctx,
  ...
  Get(const ReadOptions &options, std::string_view key) noexcept;

private:
  SSTable(IOContext &io_ctx, int fd, uint64_t file_size,
          const TableBuilderOptions &options);

  std::expected<void, Error> ReadFooterAndIndex();
  std::expected<std::unique_ptr<Block>, Error> ReadBlock(uint64_t offset,
                                                         uint64_t size) const;
