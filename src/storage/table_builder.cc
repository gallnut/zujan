...
struct TableBuilder::Rep {
  ...
  Rep(const TableBuilderOptions &opt, IOContext &io, int f)
      : options(opt), io_ctx(io), fd(f), offset(0), num_entries(0),
        pending_index_entry(false), closed(false) {
    ...
    if (opt.filter_policy != nullptr) {
      filter_block =
          std::make_unique<FilterBlockBuilder>(opt.filter_policy.get());
    }
  }
};

TableBuilder::TableBuilder(const TableBuilderOptions &options,
                           IOContext &io_ctx, int fd)
...
void TableBuilder::ChangeOptions(const TableBuilderOptions &options) {
  ...
  if (options.filter_policy != nullptr) {
    rep_->filter_block =
        std::make_unique<FilterBlockBuilder>(options.filter_policy.get());
  } else {
...
void TableBuilder::Flush() {
  ...
  auto res = rep_->io_ctx.WriteAligned(
      rep_->fd, std::span<const char>(raw.data(), raw.size()), rep_->offset);
...
std::expected<void, Error> TableBuilder::Finish() {
  ...
  if (rep_->filter_block != nullptr) {
    ...
    auto res = rep_->io_ctx.WriteAligned(
        rep_->fd,
        std::span<const char>(filter_content.data(), filter_content.size()),
        rep_->offset);
    if (!res)
  ...
  auto idx_res = rep_->io_ctx.WriteAligned(
      rep_->fd,
      std::span<const char>(index_content.data(), index_content.size()),
      rep_->offset);
  if (!idx_res)
  ...
  auto footer_res = rep_->io_ctx.WriteAligned(
      rep_->fd, std::span<const char>(footer.data(), footer.size()),
      rep_->offset);
  if (!footer_res)
    return std::unexpected(footer_res.error());
