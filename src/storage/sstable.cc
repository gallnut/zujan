...
SSTable::SSTable(IOContext &io_ctx, int fd, uint64_t file_size,
...
std::expected<std::unique_ptr<SSTable>, Error>
SSTable::Open(const TableBuilderOptions &options, IOContext &io_ctx,
              const std::string &filepath) {
  int fd = ::open(filepath.c_str(), O_RDONLY);
  if (fd < 0)
    return std::unexpected(
        Error{ErrorCode::IOError, "Failed to open SSTable file"});

  struct stat st;
  if (::fstat(fd, &st) != 0) {
    ::close(fd);
    return std::unexpected(
        Error{ErrorCode::IOError, "Failed to stat SSTable file"});
  }

  auto table =
      std::unique_ptr<SSTable>(new SSTable(io_ctx, fd, st.st_size, options));
...
std::expected<void, Error> SSTable::ReadFooterAndIndex() {
  if (file_size_ < 48)
    return std::unexpected(
        Error{ErrorCode::IOError, "File too short to be an SSTable"});
  ...
  if (n != 48)
    return std::unexpected(Error{ErrorCode::IOError, "Failed to read footer"});

  uint64_t magic = DecodeFixed64(footer_buf + 40);
  if (magic != 0xdb4775248b80fb57ull)
    return std::unexpected(Error{ErrorCode::IOError, "Bad magic number"});
  ...
  if (filter_size > 0 && options_.filter_policy) {
    ...
    if (block_res) {
      filter_data_ = std::string(
          (*block_res)->NewIterator()->value()); // Hack: get raw data string
      ...
      delete[] fbuf;
      filter_ = std::make_unique<FilterBlockReader>(
          options_.filter_policy.get(), filter_data_);
    }
  }
...
std::expected<std::unique_ptr<Block>, Error>
SSTable::ReadBlock(uint64_t offset, uint64_t size) const {
  ...
  public:
    HeapBlock(char *buf, size_t sz)
        : Block(std::string_view(buf, sz)), buf_(buf) {}
...
SSTable::Get(const ReadOptions &options, std::string_view key) noexcept {
    ...
    if (options_.block_cache && options.fill_cache) {
      ...
      if (cache_handle != nullptr) {
        block = reinterpret_cast<Block *>(
            options_.block_cache->Value(cache_handle));
      } else {
        ...
        cache_handle =
            options_.block_cache->Insert(cache_key, block, block->size(),
...
// Manager
SSTableManager::SSTableManager(IOContext &io_ctx,
                               const TableBuilderOptions &opt)
    : io_ctx_(io_ctx), options_(opt) {}

std::expected<std::optional<std::string>, Error>
SSTableManager::Get(const ReadOptions &options, std::string_view key) noexcept {
